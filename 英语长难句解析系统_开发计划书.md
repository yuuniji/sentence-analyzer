...
│
├── 3.1 选择模型（gemini-2.5-pro / gemini-2.5-flash）
├── 3.2 配置参数
...
主干提取用于生成 `<details><summary>` 的折叠标题，须在调用主解析 API 之前完成：
...
| 模型 | 适用场景 | 优势 | 劣势 |
|------|----------|------|------|
| gemini-2.5-pro | 复杂长句、段落解析 | 推理深度最强、上下文窗口大 | 速度较慢、成本较高 |
| gemini-2.5-flash | 标准单句解析 | 速度快、成本低、质量稳定 | 超复杂句子偶有遗漏 |
| gemini-2.5-flash-8b | 批量处理、词汇提取 | 极速、极低成本 | 仅适合简单任务 |

**推荐策略：** 默认使用 `gemini-2.5-flash`，用户可在设置中切换为 `gemini-2.5-pro` 处理特别复杂的句子。
